...
These figures replace slightly different ones published earlier, and the
reason is worth stating: **the benchmark harness was not reproducible.** It
selected tables with `hash(template_name)`, and Python randomizes string
hashing per process, so the seed above was doing nothing and every run scored
a different case set. Two runs of identical code produced 100% and 98.4%.
The selection is now positional and the table list is sorted; three
consecutive runs now produce byte-identical case sets. A number you cannot
reproduce is not a measurement, whichever direction it errs in.
